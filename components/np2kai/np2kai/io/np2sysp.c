...
static void np2sysp_getconfig(const void *arg1, long arg2) {
  ...
  case NP21W_SWITCH_SETDEVICEIRQ: {
    ...
    case NP21W_SWITCH_SETDEVICEIRQ_SND_MATEX:
      if (g_nSoundID == SOUNDID_MATE_X_PCM || g_nSoundID == SOUNDID_WSS_SB16 ||
          g_nSoundID == SOUNDID_PC_9801_86_WSS ||
          g_nSoundID == SOUNDID_PC_9801_86_WSS_SB16) {
        configvalue = cs4231.dmairq;
      }
    ...
    case NP21W_SWITCH_SETDEVICEIRQ_SND_SB16:
      if (g_nSoundID == SOUNDID_SB16 || g_nSoundID == SOUNDID_WSS_SB16 ||
          g_nSoundID == SOUNDID_PC_9801_86_SB16 ||
          g_nSoundID == SOUNDID_PC_9801_86_WSS_SB16 ||
          g_nSoundID == SOUNDID_PC_9801_118_SB16 ||
          g_nSoundID == SOUNDID_PC_9801_86_118_SB16) {
    ...
    case NP21W_SWITCH_SETDEVICEIRQ_SND_SPB:
      if (g_nSoundID == SOUNDID_SPEAKBOARD ||
          g_nSoundID == SOUNDID_SPARKBOARD) {
        configvalue = g_opna[0].s.irq;
...
static void np2sysp_cngconfig(const void *arg1, long arg2) {
  ...
  case NP21W_SWITCH_SYNCCLOCK:
    ...
    if (configvalue == 1) {
      ...
      hrtimertimeuint =
          (((UINT32)hrtimertime.hour * 60 + (UINT32)hrtimertime.minute) * 60 +
           (UINT32)hrtimertime.second) *
              32 +
          ((UINT32)hrtimertime.milli * 32) / 1000;
    ...
    if (configvalue != (UINT8)g_nSoundID) {
      ...
      if (((pccore.model & PCMODELMASK) >= PCMODEL_VX) &&
        ...
        if (g_nSoundID == SOUNDID_MATE_X_PCM ||
            ((g_nSoundID == SOUNDID_PC_9801_118 ||
              g_nSoundID == SOUNDID_PC_9801_86_118 ||
              g_nSoundID == SOUNDID_PC_9801_118_SB16 ||
              g_nSoundID == SOUNDID_PC_9801_86_118_SB16) &&
  ...
  case NP21W_SWITCH_SETDEVICEIRQ: {
    ...
    case NP21W_SWITCH_SETDEVICEIRQ_SND_86:
      ...
      break;
    case NP21W_SWITCH_SETDEVICEIRQ_SND_118:
      if (irqvalue == 0x03 || irqvalue == 0x06 || irqvalue == 0x0a ||
          irqvalue == 0x0c || irqvalue == 0xff) {
...
#if defined(SUPPORT_SOUND_SB16)
    case NP21W_SWITCH_SETDEVICEIRQ_SND_SB16:
      if (irqvalue == 3 || irqvalue == 5 || irqvalue == 10 || irqvalue == 12 ||
          irqvalue == 0xff) {
        g_sb16.dmairq = 3;
  ...
  if (mode == 0 || mode == 1 || mode == 2 || mode == 3) {
...
#endif
    {
      ...
      if (mode == 1) {
        ...
        CPU_DI |=
            0x8000; // 絶対座標モード　本当はAXレジスタだが触れないのでDIで
        CPU_BX = mouseX; // X座標
        CPU_CX = mouseY; // Y座標
    ...
    SINT16 x, y;
    UINT8 btn = mousemng_getstat(
        &x, &y, mode == 5 ? 1 : 0); // モード5は取得した座標をクリアする
    CPU_BX = x;                     // X座標
    CPU_CX = y;                     // Y座標
    CPU_DI = btn;                   // ボタンの状態
    CPU_FLAG |= C_FLAG;             // 成功ならC_FLAGを立てる
...
static const char cmd_hdrvsetn[] =
    "setn_hostdrv"; // Set new protocol np21w ver0.86 rev95
