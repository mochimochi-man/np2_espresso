...
volatile int g_np2_snd_locks = 0;
...
static void streamprepare(UINT samples) {
	...
	if (count < samples) g_np2_snd_skip += (int)(samples - count);   // ESP32 diag
	if (count) {
		g_np2_snd_gen += (int)count;   // ESP32 diag
...
const SINT32 *ret;
	...
	if (ret) g_np2_snd_locks++;   // ESP32 diag
	if (ret) {
		SNDCSEC_ENTER;
