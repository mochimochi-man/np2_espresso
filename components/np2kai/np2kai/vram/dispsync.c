...
BOOL dispsync_renewalvertical(void) {
	...
	/* ESP32: the vertical back-porch comes straight from emulated GDC registers,
	   which are garbage during early boot. Unclamped, text_vbp/grph_vbp make
	   textvad (= vbp*SURFACE_WIDTH) and the ZeroMemory below run past the tight
	   np2_tram/np2_vram buffers (SURFACE_SIZE), corrupting adjacent PSRAM heap.
	   Clamp so every buffer write stays in bounds. */
	if (text_vbp > SURFACE_HEIGHT) {
		text_vbp = SURFACE_HEIGHT;
	}
	if (grph_vbp > SURFACE_HEIGHT) {
		grph_vbp = SURFACE_HEIGHT;
	}
