...
static UINT8 scrndraw_draw_impl(UINT8 redraw) {
...
// ESP32 diag: time the composite (declared in pccore.c)
#include "esp_timer.h"
extern volatile long long g_prof_draw;
UINT8 scrndraw_draw(UINT8 redraw) {
    int64_t _t = esp_timer_get_time();
    UINT8 _r = scrndraw_draw_impl(redraw);
    g_prof_draw += esp_timer_get_time() - _t;
    return _r;
}
