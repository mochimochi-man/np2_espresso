...
static void b0clr(void) { b0p.flg = FALSE; }
#endif
...
static REG16 boot_hd(REG8 drv) {
  ...
  if (pccore.hddif & PCHDD_IDE) {
    ret =
        sxsi_read((drv & 0xf0) == 0x80 ? (0x80 | sxsi_unittbl[drv & 0x3]) : drv,
                  0, mem + 0x1fc00, 0x400);
  } else {
...
REG16 bootstrapload(void) {
  ...
  if (pccore.hddif & PCHDD_IDE) {
    for (i = 0; (i < 4) && (!bootseg); i++) {
      if (sxsi_getptr(sxsi_unittbl[i])->devtype == SXSIDEV_CDROM &&
          (sxsi_getptr(sxsi_unittbl[i])->flag & SXSIFLAG_READY)) {
...
void bios0x1b(void) {
  ...
  if (devtype != 0x20 && devtype != 0xa0 && devtype != 0xc0 &&
      (useidebios ||
       (devtype != 0x80 &&
        devtype != 0x00))) // エミュレーションSASI, IDE　あるいは SCSIは除外
  {
    REG8 seg;
    UINT sp;
  ...
  if (devtype == 0xc0) {
    TRACEOUT(("%.4x:%.4x AX=%.4x BX=%.4x CX=%.4x DX=%.4 ES=%.4x BP=%.4x",
              MEMR_READ16(CPU_SS, CPU_SP + 2), MEMR_READ16(CPU_SS, CPU_SP),
...
UINT bios0x1b_wait(void) {
  ...
  UINT addr;
  REG8 bit;
  static UINT32 int_timeout =
      0; // np21w ver0.86 rev51 Win3.1用 暫定無限ループ回避
