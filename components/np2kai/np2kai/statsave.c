...
enum {
  ...
  STATFLAG_BWD_COMPATIBLE =
      0x4000, // このフラグが立っているとき、古いバージョンのステートセーブと互換性がある（足りないデータは0で埋められるので注意する）いまのところSTATFLAG_BINのみサポート
  STATFLAG_FWD_COMPATIBLE =
      0x8000, // このフラグが立っているとき、新しいバージョンのステートセーブと互換性がある（足りないデータは無かったことになるので注意する）いまのところSTATFLAG_BINのみサポート
};
...
static SFFILEH statflag_open(const OEMCHAR *filename, OEMCHAR *err,
                             int errlen) {
...
static int statflag_writepath(STFLAGH sfh, const OEMCHAR *path, UINT ftype,
                              int readonly) {
...
static UINT GetSoundFlags(SOUNDID nSoundID) {
  ...
  case SOUNDID_PC_9801_86_118_SB16:
    return FLAG_OPNA1 | FLAG_OPNA2 | FLAG_PCM86 | FLAG_CS4231 | FLAG_OPL3 |
           FLAG_SB16;
  ...
  case SOUNDID_PX2:
    return FLAG_OPNA1 | FLAG_OPNA2 | FLAG_OPNA3 | FLAG_OPNA4 | FLAG_OPNA5 |
           FLAG_PCM86;
#endif
  ...
  if (nSoundID == SOUNDID_INVALID) {
    ...
    if (nSaveFlags & FLAG_MG) {
      ...
      if (datalen > sizeof(g_musicgen))
        return STATFLAG_FAILURE; // 旧バージョンで読めないようにしておく（コメントアウトすると読めるようになるが、ちゃんと考えて設計しないと危険）
      ret |= statflag_read(sfh, &g_musicgen, MIN(datalen, sizeof(g_musicgen)));
      if (datalen > sizeof(g_musicgen)) {
        sfh->pos += datalen - sizeof(g_musicgen);
      } else {
        memset((UINT8 *)(&g_musicgen) + datalen, 0,
               sizeof(g_musicgen) - datalen); // ない部分は0埋め
      }
      board14_allkeymake();
    }
    for (i = 0; i < NELEMENTS(g_opna); i++) {
    ...
    if (nSaveFlags & FLAG_PCM86) {
      ...
      if (datalen > sizeof(g_pcm86)) {
        ...
        memset((UINT8 *)(&g_pcm86) + datalen, 0,
               sizeof(g_pcm86) - datalen); // ない部分は0埋め
      }
    }
    if (nSaveFlags & FLAG_CS4231) {
      ret |= statflag_read(sfh, &datalen, sizeof(datalen));
      ...
      if (datalen > sizeof(cs4231)) {
        ...
        memset((UINT8 *)(&cs4231) + datalen, 0,
               sizeof(cs4231) - datalen); // ない部分は0埋め
      }
    }
    if (nSaveFlags & FLAG_AMD98) {
    ...
    if (nSaveFlags & FLAG_SB16) {
      ...
      if (datalen > sizeof(g_sb16)) {
        ...
        memset((UINT8 *)(&g_sb16) + datalen, 0,
               sizeof(g_sb16) - datalen); // ない部分は0埋め
      }
    }
#endif
  } else {
    ...
    if (nSaveFlags & FLAG_MG) {
      ...
      if (sizeof(MUSICGEN_OLD) < sizeof(g_musicgen)) {
        memset((UINT8 *)(&g_musicgen) + sizeof(MUSICGEN_OLD), 0,
               sizeof(g_musicgen) - sizeof(MUSICGEN_OLD)); // ない部分は0埋め
      }
      board14_allkeymake();
    }
    for (i = 0; i < NELEMENTS(g_opna); i++) {
    ...
    if (nSaveFlags & FLAG_PCM86) {
      ...
      if (sizeof(_PCM86_OLD) < sizeof(g_pcm86)) {
        memset((UINT8 *)(&g_pcm86) + sizeof(_PCM86_OLD), 0,
               sizeof(g_pcm86) - sizeof(_PCM86_OLD)); // ない部分は0埋め
      }
    ...
    if (nSaveFlags & FLAG_CS4231) {
      ...
      if (sizeof(_CS4231_OLD) < sizeof(cs4231)) {
        memset((UINT8 *)(&cs4231) + sizeof(_CS4231_OLD), 0,
               sizeof(cs4231) - sizeof(_CS4231_OLD)); // ない部分は0埋め
      }
    }
    if (nSaveFlags & FLAG_AMD98) {
      ret |= amd98_sfload(sfh, tbl);
    ...
    if (nSaveFlags & FLAG_SB16) {
      ...
      if (SIZEOF_SB16_OLD < sizeof(g_sb16)) {
        memset((UINT8 *)(&g_sb16) + SIZEOF_SB16_OLD, 0,
               sizeof(g_sb16) - SIZEOF_SB16_OLD); // ない部分は0埋め
      }
    }
#endif
  }
...
int statsave_check(const OEMCHAR *filename, OEMCHAR *buf, int size) {
  ...
  while ((!done) && (ret != STATFLAG_FAILURE)) {
    ...
    tblterm = tbl + NELEMENTS(np2tbl);
    while (tbl < tblterm) {
      if (!memcmp(sffh->sfh.hdr.index, tbl->index,
                  sizeof(sffh->sfh.hdr.index))) {
        break;
      }
      tbl++;
    }
...
int statsave_load_d(void) {
  ...
  // PCCORE read!
  ret = statflag_readsection(sffh);
  if ((ret != STATFLAG_SUCCESS) || (memcmp(sffh->sfh.hdr.index, np2tbl[0].index,
                                           sizeof(sffh->sfh.hdr.index)))) {
    statflag_close(sffh);
    return (STATFLAG_FAILURE);
  }
  ...
  if (pccore.hddif & PCHDD_IDE) {
    ...
    for (; idx < 4; idx++) {
      sxsi_unittbl[idx] =
          ncidx; // XXX: 余ったDA/UAはとりあえず未接続の番号に設定
    }
  }
#endif
  ...
  if (g_nSoundID == SOUNDID_WAVESTAR) {
    ...
    if (usefmgen) {
      ...
      opna_fmgen_setallvolumePSG_linear(np2cfg.vol_ssg * np2cfg.vol_master /
                                        100);
      opna_fmgen_setallvolumeRhythmTotal_linear(np2cfg.vol_rhythm *
                                                np2cfg.vol_master / 100);
    }
