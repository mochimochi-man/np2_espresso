...
static BRESULT setidentify(IDEDRV drv) {
  ...
  if ((sxsi == NULL) ||
      (!(sxsi->flag & SXSIFLAG_READY) && drv->device != IDETYPE_CDROM)) {
    return (FAILURE);
  }
  ...
  if (drv->device == IDETYPE_HDD) {
    ...
    tmp[10 + 2] =
        '0' + drv->sxsidrv; // シリアル番号はユニークにしておかないと駄目っぽい
    tmp[22] = 4;
    for (i = 0; i < 4; i++) {
    ...
    if (sxsi->totals > 65535 * 16 * 255) {
      ...
      tmp[102] =
          (UINT16)(sxsi->totals >>
                   32); // Maximum user LBA for 48-bit Address feature set 3
      tmp[103] =
          (UINT16)(sxsi->totals >>
                   48); // Maximum user LBA for 48-bit Address feature set 4
...
void ideioint(NEVENTITEM item) {
  ...
  if (!(dev->drv[0].ctrl & IDECTRL_NIEN) ||
...
static void setdintr(IDEDRV drv, UINT8 errno, UINT8 status, UINT32 delay) {
  ...
  if (!(drv->ctrl & IDECTRL_NIEN)) {
    ...
    //// 指定した時間遅延（マイクロ秒）
    // nevent_set(NEVENT_SASIIO, (pccore.realclock / 1000 / 1000) * delay,
    // ideioint, NEVENT_ABSOLUTE);
...
static void drvreset(IDEDRV drv) {
  ...
  if (drv->device == IDETYPE_CDROM) {
    ...
    drv->cy = 0x0000;
#if defined(SUPPORT_IDEIO_48BIT)
    drv->lba48[1] = drv->lba48[2] = drv->lba48[3] = drv->lba48[4] =
        drv->lba48[5] = 0;
    drv->lba48[0] = 1;
    drv->lba48sc = 0x0001;
#endif
...
static void writesec(IDEDRV drv) {
  ...
  if (drv->mulcnt ==
      0) { // WRITE MULTIPLEの時はブロック書き込み完了時に割り込み ->
           // 最後の割り込みはideio_w16()内に存在
    if (ideio.wwait > 0) {
      drv->status |= IDESTAT_BSY;
      drv->status &= ~IDESTAT_DRQ;
...
static void IOOUTCALL ideio_o430(UINT port, REG8 dat) {
  ...
  TRACEOUT(("ideio setbank%d %.2x [%.4x:%.8x]", (port >> 1) & 1, dat, CPU_CS,
            CPU_EIP));
#if defined(NP2_WIN)
...
static REG8 IOINPCALL ideio_i430(UINT port) {
  ...
  if ((port >> 1) & 1) {
    ...
    // 430h
    int compmode = (sxsi_getdevtype(0) != SXSIDEV_CDROM &&
                    sxsi_getdevtype(1) != SXSIDEV_CDROM &&
                    sxsi_getdevtype(2) == SXSIDEV_CDROM &&
                    sxsi_getdevtype(3) != SXSIDEV_CDROM); // 旧機種互換モード？
    IDEDEV dev;
    dev = getidedev();
    //
    if (dev) {
  ...
  TRACEOUT(("ideio getbank%d %.2x [%.4x:%.8x]", (port >> 1) & 1, ret, CPU_CS,
            CPU_EIP));
  return (ret & 0x7f);
}
...
static REG8 IOINPCALL ideio_i433(UINT port) {
  ...
  if (ret == 0x2 && ideio.dev[1].drv[0].device == IDETYPE_NONE &&
      ideio.dev[1].drv[1].device == IDETYPE_NONE) {
    ret = 0;
  }
...
static REG8 IOINPCALL ideio_i435(UINT port) { return (0x00); }
...
static void IOOUTCALL ideio_o64c(UINT port, REG8 dat) {
  ...
  if (dev->drivesel != drvnum) {
    // dev->drv[drvnum].status = dev->drv[drvnum].status &
    // ~(IDESTAT_DRQ|IDESTAT_BSY); drvreset(&(dev->drv[drvnum]));
...
static void IOOUTCALL ideio_o64e(UINT port, REG8 dat) {
  ...
  case 0x1f:
    ...
    if (drv->device == IDETYPE_HDD) {
      ...
      drv->cy = 0x0000;
#if defined(SUPPORT_IDEIO_48BIT)
      drv->lba48[0] = drv->lba48[1] = drv->lba48[2] = drv->lba48[3] =
          drv->lba48[4] = drv->lba48[5] = 0;
#endif
      // if (!(drv->dr & IDEDEV_LBA)) {
      //	drv->sn = 0x01;
      // }
...
static REG8 IOINPCALL ideio_i74c(UINT port) {
  ...
  if (drv) {
    TRACEOUT(
        ("ideio alt status %.2x [%.4x:%.8x]", drv->status, CPU_CS, CPU_EIP));
    return (drv->status);
  } else {
    return (0xff);
  }
...
void IOOUTCALL ideio_w16(UINT port, REG16 value) {
  ...
  dev = getidedev();
  drv = getidedrv();
  if ((drv != NULL) && (drv->status & IDESTAT_DRQ) &&
      (drv->bufdir == IDEDIR_OUT)) {
    ...
    if (drv->bufpos >= drv->bufsize) {
      ...
      case 0xa0:
        ...
        drv->status &= ~(IDESTAT_BSY | IDESTAT_SERV | IDESTAT_CHK |
                         IDESTAT_DRQ); // clear DRQ bit np21w ver0.86 rev38
        drv->status |= IDESTAT_DRDY | IDESTAT_DSC;
        drv->error = 0;
        setintr(drv);
        break;
...
static BRESULT SOUNDCALL playdevaudio(IDEDRV drv, SINT32 *pcm, UINT count) {
  ...
  //	samplen_d /= 100;
  // }
  // if(g_nSoundID == SOUNDID_PC_9801_118 || g_nSoundID == SOUNDID_MATE_X_PCM ||
    ...
    if (r) {
      ...
      if (samplen_n < samplen_d) {
        ...
        do {
          ...
          buf_l += (SINT)((int)(sampl) *
                          ((int)np2cfg.davolume * np2cfg.vol_master / 100) *
                          cdda_softvolume_L / 255 / 31);
          buf_r += (SINT)((int)(sampr) *
                          ((int)np2cfg.davolume * np2cfg.vol_master / 100) *
                          cdda_softvolume_R / 255 / 31);
          buf_count++;
          if (sampcount2_n > sampcount2_d) {
            pcm[0] += buf_l / buf_count;
            pcm[1] += buf_r / buf_count;
            // pcm[0] += (SINT)((int)(sampl)*((int)np2cfg.davolume *
            // np2cfg.vol_master / 100)/255); pcm[1] +=
            // (SINT)((int)(sampr)*((int)np2cfg.davolume * np2cfg.vol_master /
            // 100)/255);
        ...
        do {
          sampl = ((SINT8)ptr[1] << 8) + ptr[0];
          sampr = ((SINT8)ptr[3] << 8) + ptr[2];
          pcm[0] += (SINT)((int)(sampl) *
                           ((int)np2cfg.davolume * np2cfg.vol_master / 100) *
                           cdda_softvolume_L / 255 / 31);
          pcm[1] += (SINT)((int)(sampr) *
                           ((int)np2cfg.davolume * np2cfg.vol_master / 100) *
                           cdda_softvolume_L / 255 / 31);
...
static void devinit(IDEDRV drv, REG8 sxsidrv) {
  ...
  //	}
  // #endif
  // memset(ideio_mediastatusnotification, 0,
  // sizeof(ideio_mediastatusnotification)); memset(ideio_mediachangeflag, 0,
  // sizeof(ideio_mediachangeflag));
}

void ideio_initialize(void) { atapi_initialize(); }

void ideio_deinitialize(void) { atapi_deinitialize(); }
...
void ideio_basereset() {
  ...
  if (pccore.hddif & PCHDD_IDE) {
    ...
    if (compmode) {
      mem[0x05ba] = (sxsi_getdevtype(3) == SXSIDEV_HDD ? 0x8 : 0x0) |
                    (sxsi_getdevtype(2) == SXSIDEV_HDD ? 0x4 : 0x0) |
                    (sxsi_getdevtype(1) == SXSIDEV_HDD ? 0x2 : 0x0) |
                    (sxsi_getdevtype(0) == SXSIDEV_HDD ? 0x1 : 0x0);
    } else {
      mem[0x05ba] = (sxsi_getdevtype(3) != SXSIDEV_NC ? 0x8 : 0x0) |
                    (sxsi_getdevtype(2) != SXSIDEV_NC ? 0x4 : 0x0) |
                    (sxsi_getdevtype(1) != SXSIDEV_NC ? 0x2 : 0x0) |
                    (sxsi_getdevtype(0) != SXSIDEV_NC ? 0x1 : 0x0);
    }
  }
}
        ...
        for (i = 0; i < 4; i++) {
          ...
          if ((sxsi != NULL) && (np2cfg.idetype[i] == SXSIDEV_HDD) &&
            ...
            if (sxsi->surfaces != 8 || sxsi->sectors != 17) {
              TRACEOUT(("Incompatible CHS parameter detected. IDE BIOS "
...
void ideio_bind(void) {
  ...
  if (pccore.hddif & PCHDD_IDE) {
    ...
    iocore_attachout(
        0x1e8e,
        ideio_o1e8e); // 一部IDE
                      // BIOSはこれがないと起動時にフリーズしたりシリンダ数が0になる
    iocore_attachinp(
        0x1e8e,
        ideio_i1e8e); // 一部IDE
                      // BIOSはこれがないと起動時にフリーズしたりシリンダ数が0になる
